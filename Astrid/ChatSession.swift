...
//  - Session 6B: Chat session identity + immutable snapshot inputs.
//  - Session 6C: Per-session transcripts and persistence support.
//  - Session 7A: History eligibility + sorting via `lastActivityAt`.
//  - Session 7B: One-shot neutral chat titles (persisted, never regenerated).
//  - Session 7.5: Manual deletion support (model participates in removal + persistence).
//
//  Explicitly out of scope (by design):
//  - UI state (typing indicator, scroll position, focus, selection).
//  - Chat history UI or sidebar behavior.
//  - Persistence mechanics or file I/O (handled by ChatViewModel).
//
//  Design intent:
//  - Session identity must be stable and immutable once created.
//  - Profile/system prompt changes must never affect an active session.
//  - This model must remain UI-agnostic and slow-changing.
//
//
//
