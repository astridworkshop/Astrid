...
//
//  Detail column responsibilities (Milestone 1):
...
//
//  Sidebar responsibilities (Milestone 1 + Session 8A):
...
//      - History selection (Session 7A) + session switching / in-flight request cancellation.
//      - Manual deletion (Session 7.5) request + confirmation + immediate persistence.
...
//      - Title selection (Session 7B): persisted neutral title preferred, deterministic fallbacks.
//
//  Session 8A note:
//  - Visual sidebar polish (harmonious background, independent history scrolling + peek affordance)
//    now lives in `AstridSidebarView`.
//
//  Rendering notes:
...
"""
    ...
    private static func parseMarkdownBlocks(_ input: String) -> [MarkdownBlock] {
        // Normalize newlines and trim trailing whitespace (but keep meaningful indentation within code).
        ...
        func bulletMatch(_ rawLine: String) -> (level: Int, text: String)? {
            // Supports: *, -, and common LLM spacing like "*   item"
            let spaces = leadingSpacesCount(rawLine)
            let trimmed = rawLine.trimmingCharacters(in: .whitespaces)
            guard trimmed.hasPrefix("* ") || trimmed.hasPrefix("-") || trimmed.hasPrefix("*\t") || trimmed.hasPrefix("- ") else { return nil }
        ...
        .sheet(isPresented: $showProfilesManager) {
            NavigationStack {
                ProfilesManagerView(
                    ...
                    onEdit: { beginEditProfile(id: $0) },
                    onCreate: { beginCreateProfile() }
                )
