...
//  Used by ContentView and AstridSidebarView to drive session lists, message display, and actions.
...
//  - Session 6B: Chat session identity + snapshot foundation.
//  - Session 6C: Per-session transcripts + persistence (load/save).
//  - Session 7A: History switching support (select session + reset runtime-only state).
//  - Session 7B: One-shot neutral title generation after first assistant reply (non-blocking, persisted).
//  - Session 7.5: Manual chat deletion (remove session + persist immediately; reset if active).
//
...
//  - Milestone 1 behaviors (scroll stability, typing indicator rules, splash rules) must not regress.
//
//
//
//
